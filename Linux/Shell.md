...
7. 系统资源查看
   ...
   7. uname [选项]
      ...
      2. -r 查看内核版本
      3. -s 查看内核名称
   8. 借用file命令查看一个外部命令会顺带输出系统位数 file /bin/ls
   9. lsb_release -a 查看系统发行版本
   10. lsof [选项] 列出进程调用文件
       1. -c 字符串 只列出字符串开头的进程开头的文件
...
1. rsyslog和syslog是兼容的，是centos6.3后的日志系统
2. 常见日志的作用
   1. /var/log/cron 记录系统定时任务相关的日志
   2. /var/log/cups 记录打印信息的日志
   3. /var/log/dmesg 记录系统开机时自检的信息
   4. /var/log/btmp 记录错误登录的日志 二进制，要使用lastb查看
   5. /var/log/lastlog 也是二进制，记录最后一次登录时间的日志
   6. /var/log/mailog 记录邮件信息
   7. /var/log/message 记录系统重要信息日志，如果有问题要先查看这个日志
   8. /var/log/secure 记录验证、授权方面的信息
   9. /var/log/wtmp 永久记录所有用户的登录、注销信息
   10. /var/run/utmp 记录当前已经登录的用户的信息
3. 日志文件基本格式
   1. 事件产生的时间
   2. 发生事件的服务器的主机名
   3. 产生事件的服务名或程序名
   4. 事件的具体信息
4. /etc/rsyslog.conf配置文件
   1. 服务名称[连接符号]日志等级 日志记录位置
      1. \* 所有日志等级
      2. . 只要比后面的等级高的日志都要记录下来
      3. .= 等于这个等级的日志
      4. .! 不记录该级别日志
   2. 服务名称
      1. auth 安全和认证相关
      2. authpriv 安全和认证相关（私有）
      3. cron 系统定时任务 cront和at产生的日志
      4. daemon 和各个守护进程相关的日志
      5. ftp ftp守护进程日志
      6. kern 内核产生的日志
...
5. 日志轮替
   1. 明明规则
      1. 日志文件的命名规则 拥有dateext参数 用日期作为日志文件后缀 例如secure-20130605
      2. 如果配置文件没有dateext参数，仍然会轮替
   2. /etc/logrotate.conf 日志轮替配置文件
      1. daily 日志轮替周期是每天
      2. weekly 每周
      3. monthly 每月
      4. rotate 数字 保留日志文件的个数 0指没有备份
      5. compress 日志轮替时，旧日志进行压缩
      6. create mode owner group 建立新日志组
      7. mail address 邮件发送
      8. missingok 日志不存在 忽略警告信息
      9. notifempty 日志为空不轮替
      10. minsize 大小 日志轮替最小值
      11. size 大小 日志只有大于指定大小进行轮替
      12. datext 使用日期作为后缀
   3. logrotate [选项] 配置文件名
      1. 如果此命令没有选项，则会按照配置文件中的条件进行轮替
      2. -v 显示日志轮替过程
      3. -f 强制进行日志轮替
...
1. 启动流程
  ...
  ![image-20220606133351206](Shell.assets/image-20220606133351206-16544936363551.png)

1. initramfs内存文件系统是centos6.3以后的方式
...
1. Linux备份目录
   ...
   5. 其他目录
2. 备份命令
   1. dump命令 dump [选项] 备份之后的文件名 源文件或目录
      1. -level 0~9 十个备份级别 -0
      2. -f 文件名 指定备份之后的文件名
      3. -u 备份成功之后，把备份时间记录在/etc/dumpdates文件
      4. -v 显示备份过程中更多的输出信息
      5. -j 调用bzlib库亚索备份文件，其实就是把备份文件亚索为.bz2格式
      6. -W 显示允许被dump的分区的备份等级及备份时间
   2. restore命令 restore [模式选项] [选项]
      1. -C 比较备份数据和实际数据变化
      2. -i 进入交互模式
      3. -t 查看模式
      4. -r 还原模式
